...
class FileHeader {
    ...
    public  boolean extendHeader(BitMap freeMap, int extraSectors, int extraSize) {
	if (freeMap.numClear() < extraSectors || numSectors + extraSectors > NumDirect) {
	    Debug.print('f', "extendHeader: No free sectors available.\n");
	    return false;
	} else {
	    for (int i = numSectors; i <  numSectors + extraSectors; i++) {
		dataSectors[i] = freeMap.find();
	    }
	    numSectors = numSectors + extraSectors;
	    numBytes = numBytes + extraSize; 
	    return true;
	}
    }
	...
	numBytes = fileSize;
	int extraSectors  = fileSize / diskSectorSize;
	if (fileSize % diskSectorSize != 0) extraSectors++;
	
	return extendHeader(freeMap, extraSectors, fileSize);
    }
