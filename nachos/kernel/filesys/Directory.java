...
class Directory {
    ...
    /** Number of default entries in the directory. */
    ...
    void setTableSize(OpenFileReal file) {
	tableSize = 0;
	int pos = 0;
	byte buffer[] = new byte[DirectoryEntry.sizeOf()];
	file.readAt(buffer, 0, DirectoryEntry.sizeOf(), pos);
	pos += DirectoryEntry.sizeOf();
	
	while (buffer[0] != 0) {
	    tableSize++;
	    buffer = new byte[DirectoryEntry.sizeOf()];
	    file.readAt(buffer, 0, DirectoryEntry.sizeOf(), pos);
	    pos += DirectoryEntry.sizeOf();
	}
     ...
     */
    void fetchFrom(OpenFileReal file) {
     ...
     */
    boolean add(String name, int newSector, boolean isDir) { 
	if (findIndex(name) != -1)
	    return false;

	for (int i = 0; i < tableSize; i++)
	    if (!table[i].inUse()) {
		if(!table[i].setUsed(name, newSector,isDir))
		    return(false);
		return(true);
	    }
	...
	newTable[tableSize] = newEntry;
	
	table = newTable;
	if(!table[tableSize].setUsed(name, newSector,isDir))
	    return(false);
     ...
     */
    void list(String fullPath) {
	for (int i = 0; i < tableSize; i++)
	    if (table[i].inUse()) {
		if (table[i].isDir) {
		    String dirName = table[i].getName();
		    String newfullPath; 
		    if (fullPath == "/") {
			System.out.println("Directory" + ": " + fullPath + table[i].getName());
			newfullPath = fullPath + dirName;
		    }else {
			System.out.println("Directory" + ": " + fullPath + "/" + table[i].getName());
			newfullPath = fullPath + "/" + dirName;
		    }
		    OpenFileReal currDir = filesystem.openDir(newfullPath);
		    if (currDir != null) {
			Directory newDirectory = new Directory(0, filesystem);
			newDirectory.setTableSize(currDir);
			newDirectory.fetchFrom(currDir);
			newDirectory.list(newfullPath);
		    }
		} else
		    System.out.println("File" + ": " + fullPath + table[i].getName());
	    }
    }
