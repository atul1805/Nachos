...
public class FileSystemReal extends FileSystem {
  ...
  public final OpenFile freeMapFile;

  /** "Root" directory -- list of file names, represented as a file. */
  private final OpenFileReal directoryFile;
  ...
  protected FileSystemReal(DiskDriver diskDriver, boolean format) { 
    ...
    if (format) {
      ...
      Directory directory = new Directory(0, this);
  ...
  public boolean create(String name, long initialSize) {
    ...
    directory = new Directory(0, this);
    directory.setTableSize(directoryFile);
  ...
  public boolean makeDir(String path, String name) {
      Directory directory;
      BitMap freeMap;
      FileHeader hdr;
      int sector;
      ...
      Debug.printf('f', "Creating Directory %s\n", name);

      directory = new Directory(0, this);
      OpenFileReal currDir = openDir(path);
      
      if (currDir == null)
	  return false;
      
      directory.setTableSize(currDir);
      directory.fetchFrom(currDir);

      if (directory.findDir(name) != -1)
        success = false;			// file is already in directory
      else {	
        freeMap = new BitMap(numDiskSectors);
        freeMap.fetchFrom(freeMapFile);
        ...
        if (sector == -1) 		
            ...
            hdr.writeBack(sector);
            freeMap.writeBack(freeMapFile);
            directory.writeBack(currDir);
        }
      }
      return success;      
  }
  
  public OpenFileReal openDir(String name) {
      String currPath[] = getPath(name);
      
      if (currPath.equals(null))
	  return null;
      
      OpenFileReal file =  directoryFile;
      
      for (int i = 0; i < currPath.length; i++) {
	  String dir = currPath[i];
	  
	  Directory directory = new Directory(0, this);
	  directory.setTableSize(file);
	  directory.fetchFrom(file);
	  
	  if (dir != "") {
	      int sector = directory.findDir(dir);
	      if (sector == -1) {
		  return null;
	      }
	      file = new OpenFileReal(sector,this);
	  }
      }
      return file;
  }
  ...
  public boolean rmDir(String path, String name) {
      Directory directory;
      int sector;

      Debug.printf('f', "Deleting Directory %s\n", name);
      ...
      Directory subDirectory = new Directory(0, this);
      OpenFileReal subDir = openDir(path + "/" + name);
      subDirectory.setTableSize(subDir);
      subDirectory.fetchFrom(subDir);
      if (subDirectory.tableSize > 0) {
	  for (int i = 0; i < subDirectory.tableSize; i++) {
	      if (subDirectory.table[i].isDir) {
		  rmDir(path + "/" + name, subDirectory.table[i].getName());
		  
	      } else {
		  removeSector(subDirectory.table[i].sector);
		  subDirectory.remove(subDirectory.table[i].getName());
  ...
  public void printStatistics() {
      LinkedList<Integer> usedSectors = new LinkedList<Integer>();
      usedSectors.add(FreeMapSector);
      usedSectors.add(DirectorySector);
      
      getStatistics(usedSectors, "/");
      
      for (int i = 0; i < usedSectors.size(); i++) {
	  System.out.print(usedSectors.get(i) + " ");
      }
      System.out.println("");
      BitMap freeMap = new BitMap(numDiskSectors);
      freeMap.fetchFrom(freeMapFile);
      System.out.println(freeMap.numClear());
  }
  
  public void getStatistics(LinkedList<Integer> usedSectors, String path) {
      OpenFileReal file;
      if (path == "/")
	  file = directoryFile;
      else
      ...
      if (directory.tableSize > 0) {
	  for (int i = 0; i < directory.tableSize; i++) {
	      if (directory.table[i].isDir) {
		  usedSectors.add(directory.findDir(directory.table[i].getName()));
		  if (path == "/")
		      getStatistics(usedSectors, path + directory.table[i].getName());
		  else
		      getStatistics(usedSectors, path + "/" + directory.table[i].getName());
	      } else {
		  ...
		  fileHdr.fetchFrom(sector);
		  for (int j = 0; j < fileHdr.numSectors; j++) {
		      usedSectors.add(fileHdr.dataSectors[j]);
		  }
	      }
	  }
      }
